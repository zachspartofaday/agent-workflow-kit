...
try {
  ...
  await command('apply', /out of order/);
  await command('collect', /collected/);
  await command('report', /collect/);
  await stop(); start(); await send('get_state');
  await command('status', /collected/);
  await command('judge', /judged/);
  await command('apply', /applied/);
  await command('report', /"ready": true/);
  await command('check', /checked; check=passed/);
  await command('close', /complete/);
  assert.equal(JSON.parse(readFileSync(join(demo.cwd, 'examples/pi-workflow/fixture.json'), 'utf8')).ready, false, 'Writer changed source fixture');
  await stop();
  const persisted = readFileSync(demo.sessionFile, 'utf8').split('\n').filter(Boolean).map(s => JSON.parse(s));
  assert.equal(persisted.filter(e => e.customType === 'workflow-kit.demo.v1').length, 7);
  start(); await send('get_state'); await command('status', /complete/);
  writeFileSync(join(demo.cwd, 'examples/pi-workflow/fixture.json'), '{"schema":1,"lesson":"unsupported","ready":false}');
  ...
  await command('collect', /fixture shape/); await command('check', /applied candidate/);
  await command('close', /passing fixture check/);
  await command('reset', /inactive/);
  console.log('Pi 0.85.0 RPC walkthrough passed: registration, refusal, cancellation, approval, role ordering, collector restoration, frozen candidate, actual check, disk restoration, stale inputs, invalid source refusal, reset. No model calls.');
} finally {
  await stop();
